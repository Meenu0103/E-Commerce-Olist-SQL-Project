...
```
select customer_state, count(distinct customer_unique_id) as occurences
...
select 
    (count(case when order_count > 1 then 1 end) * 100.00 / count(*)) 
...
```
select avg(order_count) as avg_no_of_orders
from (
...
```
select c.customer_id, sum(oi.price) as customer_spending
from olist_customers  c
join olist_orders o
    on c.customer_id = o.customer_id
join olist_order_items oi
...
```
select 
    avg(order_delivered_customer_date - order_purchase_timestamp) as avg_date_diff
from olist_orders;
```
...
### 4. Which sellers have the best delivery performance?
...
select 
    ...
    avg(extract(epoch from 
        (o.order_delivered_carrier_date - o.order_approved_at)) / 86400
...
### 5. Which sellers have the worst delivery delays?
...
### 6. Which states experience the slowest deliveries?
...
### 7. What is the distribution of delivery times per category?
...
```
select 
    avg(review_answer_timestamp - review_creation_date) 
