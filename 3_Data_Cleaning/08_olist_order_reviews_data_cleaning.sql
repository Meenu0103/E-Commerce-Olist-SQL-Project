/* 
    OLIST ORDER REVIEWS – DATA CLEANING SCRIPT
...
-- 3. DUPLICATE CHECK for review_id (must be UNIQUE)
...
-- 4. DUPLICATE CHECK per order_id
-- Some orders wrongly have multiple reviews → DATA INCONSISTENCY
...
-- 5. VALIDATE REVIEW SCORE RANGE (must be 1 to 5)
SELECT *
FROM olist_order_reviews
...
SELECT *
FROM olist_order_reviews
WHERE LENGTH(review_comment_message) > 2000;



-- 8. MATCH REVIEWS WITH ORDERS (missing order_id problems)
SELECT *
FROM olist_order_reviews
...
- Some reviews have missing review_answer_timestamp (normal)
*/
